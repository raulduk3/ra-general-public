...
function getTasks(date) {
    ...
    // Adjust the daily note path to reflect the new format
    if (!dailyNoteFolder) {
        var dailyNotePath = moment(date).format("YYYY/MM/DD-MM-YYYY") + ".md";
    } else {
        var dailyNotePath = dailyNoteFolder + "/" + moment(date).format("YYYY/MM/DD-MM-YYYY") + ".md";
    }
...
function getMonth(tasks, month) {
	...
	for (w=1; w<7; w++) {
		...
		for (i=starts;i<starts+7;i++) {
			...
			if (!dailyNoteFolder) {
				// Update dailyNotePath to the new format
				var dailyNotePath = moment(currentDate).format("YYYY/MM/DD-MM-YYYY") + ".md";
			} else {
				var dailyNotePath = dailyNoteFolder + "/" + moment(currentDate).format("YYYY/MM/DD-MM-YYYY") + ".md";
			}
